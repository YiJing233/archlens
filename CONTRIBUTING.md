...
4. 单个案例运行 `npm run source:audit -- --input <case.json> --out <目录>`；批量案例运行 `npm run source:pipeline -- --input <案例目录> --out <目录>`，检查来源状态后再运行 `npm run case:pack -- --input <case.json> --out <目录>` 生成资料包。
